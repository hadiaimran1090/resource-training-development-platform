...
export const ResourceManagementPage: React.FC = () => {
  ...
  const [currentStatus, setCurrentStatus] = useState<'assigned' | 'bench'>('bench');
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-medium">
                {filteredResources.map((res) => (
                  <tr
                    key={res.id}
                    onClick={() => navigate(`/admin/users/${res.user_id}`)}
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setSelectedBenchUser({ id: res.user_id, name: res.user_name });
                          }}
      ...
      {isModalOpen && editingResource && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4">
            <h2 className="text-base font-extrabold text-slate-900">Edit Resource Profile</h2>
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <input
                  type="text"
                  disabled
                  value={editingResource.user_name}
                  className="w-full px-3.5 py-2 bg-slate-100 border border-slate-200 rounded-xl text-xs font-semibold text-slate-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700">Assigned Region</label>
                  <select
                    value={regionId}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700">Assigned Practice</label>
                  <select
                    value={practiceId}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700">Regional Lead</label>
                  <select
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={currentStatus}
                    onChange={(e) => setCurrentStatus(e.target.value as 'assigned' | 'bench')}
